...
const UserControllers = {
  ...
  userRegistration: async (req, res) => {
    let { firstName, lastName, email, password, from, image, google } =
      req.body.data; //agregué propiedad "google"
    try {
      ...
      if (existingUser) {
        if (existingUser.from.indexOf(from) !== -1) {
          res.json({
            success: false,
            from: "signup",
            message: "You have already signed up in this way. Please, sign in",
          });
        } else {
          ...
          if (from === "signup") {
            ...
            res.json({
              success: true,
              from: "signup",
              message: "Please verify your mail and then log In ",
            });
          } else {
            existingUser.save();
            res.json({
        ...
        if (from !== "signup") {
          ...
          res.json({
            ...
            from: "Signup",
            message:
              "Congratulations, your account has been created with" + from,
          });
        } else {
          await newuser.save();
  ...
  userSignin: async (req, res) => {
    ...
    try {
      ...
      if (!existingUser) {
        res.json({
          success: false,
          message: "Your user has not been found please register",
        });
      } else {
        if (from !== "signin") {
          let passwordMatch = existingUser.password.filter((pass) =>
          ...
          if (passwordMatch.length > 0) {
            ...
            res.json({
              ...
              success: true,
              response: { user: userData, from: from, token: token },
            });
          } else {
            res.json({
              success: false,
              form: from,
              message: "You have not register with " + from,
            });
          }
        } else {
          if (existingUser.verifiedMail) {
            ...
            if (passwordMatch.length > 0) {
              ...
              res.json({
                success: false,
                from: from,
                message: "The mail or password is incorrect",
              });
            }
          } else {
            res.json({
              success: false,
              from: from,
              message: "Email not verified, please verify it then log in",
            });
          }
        }
      }
      ...
      res.json({
        success: false,
        message: "Something wnet wrong try again in a few minutes",
      });
    }
  },
    ...
    if (!req.err) {
      res.json({
        ...
        response: {
          ...
          from: "token",
          message:
            "Welcome again " + req.user.firstName + " " + req.user.lastName,
